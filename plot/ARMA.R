...
rmr_arma<- function(fid, data, varagins){
  epsilon = varagins[1]
  W = varagins[2]
  tc = varagins[3]
  output = rmr_arma_run(fid, data,epsilon,tc, W)
  return(output)
}

rmr_arma_run <- function(fid,data,epsilon,tc,W) {
  T = dim(data)[1]
  N = dim(data)[2]
  run_ret = NULL
  ...
  for (t in 1:T) {
    ...
    if (t < T) {
      day_weight_n = rmr_arma_day_weight(data_close,data,t + 1,day_weight,W,epsilon)
...
rmr_arma_day_weight <- function(data_close, data, t1, day_weight,w,epsilon){
  if (t1 < w + 2) {
    x_t1 = data[t1 - 1,]
  } else {
    print(t1)
    #print(w)
    models = apply(data_close[(t1 - w):(t1 - 1),],2,auto.arima)
    predicted_price = NULL
    for (i in 1:ncol(data_close)) 
      {predicted_price[i] = as.numeric(forecast(models[[i]],h = 1)$mean)}
    x_t1 = predicted_price/data_close[t1 - 1,]
  }
